...
class Adm extends CI_Controller {
	...
	public function hasil_ujian_cetak() {
		...
		$a['hasil'] = $this->db->query("SELECT m_siswa.nama, m_siswa.id
										...
										INNER JOIN m_siswa ON tr_ikut_ujian.id_user = m_siswa.id
										WHERE tr_ikut_ujian.id_tes = '$uri3'")->result();*/
		$a['hasil'] =  $this->db->query("SELECT m_siswa.nama, m_soal.soal, tr_ujian.jawaban, tr_ujian.detail_jawaban
										FROM tr_ujian 
										INNER JOIN m_siswa ON tr_ujian.id_user = m_siswa.id INNER JOIN m_soal ON tr_ujian.id_soal = m_soal.id 
